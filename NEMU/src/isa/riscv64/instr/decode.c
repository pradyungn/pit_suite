...
static inline uint64_t get_instr(Decode *s) {
...
#ifdef CONFIG_CUSTOM_TENSOR
def_THelper(main_64) {
  // 3R1W format instruction
  def_INSTR_IDTAB("0???????????????????????????????????????????????????????????????", rvt_3r1w, rvt_3r1w_dispatch);
  // 2R1W format instruction
  def_INSTR_IDTAB("10??????????????????????????????????????????????????????????????", rvt_2r1w, rvt_2r1w_dispatch);
  // 1R1W format instruction
  def_INSTR_IDTAB("11??????????????????????????????????????????????????????????????", rvt_1r1w, rvt_1r1w_dispatch);
  return table_inv(s);
}
#endif
...
int isa_fetch_decode(Decode *s) {
  ...
  #ifdef CONFIG_CUSTOM_TENSOR
  else if (s->isa.instr.r.opcode6_2 == 0b01111) {
    // Not 32 bit. Here we only supports 64 bits
    uint32_t hi = instr_fetch(&s->snpc, 2);
    s->isa.instr.val |= (hi << 16);
    uint64_t val = instr_fetch(&s->snpc, 4);
    s->isa.instr.val |= (val << 32);
    idx = table_main_64(s);
  }
  #endif
  else {
